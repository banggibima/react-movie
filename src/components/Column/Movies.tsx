      ...
      <div className="mt-4 grid gap-4 md:grid-cols-4 lg:grid-cols-4 lg:max-w-none">
        {movies.length > 0 ? (
          movies.map((item: any) => (
            <div
              key={item.id}
              className="flex flex-col rounded-lg shadow overflow-hidden bg-zinc-50"
            >
              ...
              <div className="flex-1 p-4 flex flex-col justify-between">
                <div className="flex-1">
                  <p className="text-xs px-2 py-1 inline rounded-lg font-semibold bg-sky-100 text-sky-600">
                    {item.release_date === ''
                      ? ''
                      : new Date(item.release_date).toLocaleString('id-ID', {
                          year: 'numeric',
                        })}
                  </p>
          ...
          <h1 className="text-sm text-black mt-4">Movies not found.</h1>
        )}
      </div>
    </>
  );
